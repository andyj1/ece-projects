...
class hashTable {
  ...
  public:
    ...
    // another way to initialize instead of constructor:
    // -->hashTable(int size = 0):capacity(getPrime(size)), filled(0), data(capacity) { };
    ...
    int setPointer(const std::string &key, void *p);
  ...
  private:
    ...
    class hashItem {
    	public:
        // hashItem (const std::string &key_str = std::string(), const bool &occupy = false, const bool &del = false, void *p = NULL):
        //        key(key_str), isOccupied(occupy), isDeleted(del), pv(p) {};
      	std::string key;
      	bool isOccupied;
      	bool isDeleted;
      	void *pv;
